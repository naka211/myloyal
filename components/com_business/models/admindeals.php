 ...
 * @since  0.0.1
 */
class BusinessModelAdmindeals extends JModelItem
{
	/**
	 * @var array messages
	 */
	...
	public function getTable($type = 'Deals', $prefix = 'BusinessTable', $config = array())
	{
        ...
        public function getDealinmonth()
        {
            ...
            if (isset($this->dealinmonth))
            {
                    ...
                    ->where($db->quoteName('b.id') . ' = ' . $businessId . ' and endDate >=' . $firstDay . ' and endDate <=' . $lastDay);
        ...
        public function getDealsById()
	{
            ...
            if (isset($this->dealsById))
            {
                    ...
                    ->join('INNER',$db->quoteName('#__business','b'). ' ON (' . $db->quoteName('a.id') . ' = ' . $db->quoteName('b.userId') . ')')
                    ->join('INNER',$db->quoteName('#__deals','c'). ' ON (' . $db->quoteName('b.id') . ' = ' . $db->quoteName('c.businessId') . ')')
                    ->where($db->quoteName('b.id') . ' = ' . $businessId . " and " . $db->quoteName('c.id') . " = " . $id);
	...
	public function getDeals()
	{
            ...
            if (isset($this->deals))
            {
                    ...
                    ->join('INNER',$db->quoteName('#__business','b'). ' ON (' . $db->quoteName('a.id') . ' = ' . $db->quoteName('b.userId') . ')')
                    ->join('INNER',$db->quoteName('#__deals','c'). ' ON (' . $db->quoteName('b.id') . ' = ' . $db->quoteName('c.businessId') . ')')
                    ->where($db->quoteName('b.id') . ' = ' . $businessId . ' and status = 0');
        ...
        public function deleteDeals($id,$businessId)
        {
            $db = JFactory::getDbo();
            $query = $db->getQuery(true);
            $query->select('c.*')
